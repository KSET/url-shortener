...
def dashboard_context(user, **extra):
...
async def shorten_url(
    ...
    if custom_id and custom_id.strip():
        short_id = custom_id.strip().lower()
        if not re.match(r"^[a-z0-9\-_]+$", short_id):
            error_msg = "URL smije sadrzavati samo slova, brojke i crtice."
        elif short_id in FORBIDDEN_IDS:
            error_msg = "Ovaj naziv je rezerviran za sustav."
        elif db.query(models.URL).filter(models.URL.short_id == short_id).first():
            error_msg = "Ovaj prilagodeni URL je vec zauzet!"
    else:
...
async def redirect_url(short_id: str, db: Session = Depends(get_db)):
    ...
    raise HTTPException(status_code=404, detail="Link nije pronaden.")
